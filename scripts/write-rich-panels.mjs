...
const out = path.join(__dirname, "../src/components/ilan/detail/RichTabPanels.tsx");
const D = "div";
...
function AfetPanel({ property }: { property: PropertyRecord }) {
  const extras = getPropertyUiExtras(property);
  const risk = extras.riskScore;
  const faultKm = 8 + (risk % 40);
  const faultPct = Math.min(100, Math.round((faultKm / 50) * 100));
  return (
    <section className="idr-panel idr-panel--afet">
      <${D} className="idr-afet-top">
        <article className="idr-card idr-card--center">
          <ScoreRing score={100 - risk} label="Güvenlik skoru" size={140} />
        </article>
        <article className="idr-card idr-flex-1">
          <SectionTitle title="Fay hattı mesafesi" subtitle={\`~\${faultKm} km\`} />
          <${D} className="idr-progress">
            <span className="idr-progress__fill" style={{ width: \`\${faultPct}%\` }} />
          </${D}>
          <p className="idr-muted">North Anatolian fay zonu referans mesafesi (demo).</p>
        </article>
        <article className="idr-card idr-dask">
          <Shield size={28} aria-hidden />
          <h4>DASK</h4>
          <p>{property.disasterInsurance ? "Poliçe mevcut" : "Poliçe doğrulanmadı"}</p>
          <a href="#dask" className="idr-link">DASK sorgula</a>
        </article>
      </${D}>
      <SectionTitle title="Bölgesel deprem kayıtları" />
      <ul className="idr-quake-list">
        {extras.earthquakeHistory.map((q) => (
          <li key={q.date}>
            <span>{q.date}</span>
...
function GaleriPanel({ property }: { property: PropertyRecord }) {
  const base = property.images?.length ? property.images : property.heroImage ? [property.heroImage] : [];
  ...
  return (
    <section className="idr-panel idr-panel--galeri">
      ...
      {lightbox ? (
        <${D} className="idr-lightbox" role="dialog" aria-modal onClick={() => setLightbox(null)}>
...
export function RichTabPanel({ tab, property, typeExtras: _typeExtras, onGoTab }: RichTabPanelProps) {
  switch (tab) {
    ...
    case "AFET":
      return <AfetPanel property={property} />;
    case "ALTYAPI":
...
console.log("wrote", out, "bytes", fs.statSync(out).size, "import", fs.readFileSync(out)[0]);

  ...
  return (
    <div className="relative" ref={wrapRef}>
      ...
      >
        ...
        {unread > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 rounded-full bg-violet-500 text-[10px] font-bold text-white flex items-center justify-center border border-[#0a0f1e]">
            {unread > 9 ? "9+" : unread}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[min(100vw-2rem,22rem)] rounded-xl border border-white/10 bg-[#0f172a] shadow-2xl z-[80] overflow-hidden animate-scale-in">
            ...
            {items.length === 0 ? (
              ...
              items.map((n) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 border-b border-white/5 hover:bg-white/[0.04] transition-colors ${n.read ? "opacity-70" : "bg-blue-500/5"}`}
                >
          ...
          <p className="text-[10px] text-slate-600 px-3 py-2 border-t border-white/5">Demo bildirimler — gerçek zamanlı kanal bağlı değildir.</p>
        </div>
      )}
    </div>
  );

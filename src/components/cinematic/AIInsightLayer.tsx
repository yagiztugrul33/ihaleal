...
const toneStyles = {
  positive: "border-[var(--cizgi)] bg-[var(--zemin-yumusak)]",
  neutral: "border-[var(--cizgi)] bg-[var(--zemin-yumusak)]",
  caution: "border-[var(--cizgi)] bg-[var(--zemin-yumusak)]",
};
...
export function AIInsightLayer({
  ...
  return (
    <motion.section
      className={`rounded-[20px] border border-white/10 bg-gradient-to-br from-[var(--zemin-yumusak)] to-[#081120]/80 p-5 sm:p-6 ${className}`}
      initial="hidden"
      whileInView="show"
    ...
    >
      <motion.div variants={mp.staggerItem} className="flex items-center gap-2 mb-4">
        <motion.div
          className="flex h-9 w-9 items-center justify-center rounded-[10px] border border-[var(--cizgi)] bg-[var(--zemin-yumusak)]"
          animate={reduced ? undefined : { boxShadow: ["0 0 0 rgba(34,211,238,0)", "0 0 24px rgba(34,211,238,0.15)", "0 0 0 rgba(34,211,238,0)"] }}
          transition={{ duration: 3, repeat: Infinity }}
        >
          <Brain className="w-4 h-4 text-[var(--metin-ikincil)]" aria-hidden />
        </motion.div>
        <motion.div variants={mp.staggerItem}>
          <p className="text-[10px] font-normal uppercase tracking-[0.2em] text-[var(--metin-ikincil)]">AI intelligence</p>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <Icon className="w-4 h-4 text-[var(--metin-ikincil)] shrink-0 mt-0.5" aria-hidden />
                  ...
                  {insight.confidence != null ? (
                    <div className="mt-3">
                      <motion.div className="flex justify-between text-[10px] text-slate-500 mb-1">
                        <span>Guven</span>
                        <span className="text-[var(--metin-ikincil)]">{insight.confidence}%</span>
                      </motion.div>

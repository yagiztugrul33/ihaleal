...
export function RoadmapBanner({ className = "" }: RoadmapBannerProps) {
  return (
    <div
      role="note"
      className={`mb-6 flex gap-3 rounded-xl border border-sky-500/25 bg-sky-500/10 px-4 py-3 text-sm text-slate-300 ${className}`}
    >
      <Map className="mt-0.5 h-5 w-5 shrink-0 text-sky-400" aria-hidden />
      <p className="leading-relaxed">
        <span className="font-semibold text-sky-200">Hedef / yol haritası (taslak):</span> Bu sayfadaki teknik

...
export function ListingMortgageWidget({ priceTry, className = "" }: Props) {
  ...
  return (
    <div className={`rounded-[20px] border border-teal-500/25 bg-teal-500/5 p-4 ${className}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-normal text-teal-100 flex items-center gap-2">
          ...
          {estimate ? (
            <p className="mt-1 text-xs text-slate-400">
              ...
              <span className="text-teal-300 font-normal" dir="ltr">{formatTry(estimate.monthlyPayment)}/{lv.monthlyInstallment.toLowerCase().split(" ")[0]}</span>

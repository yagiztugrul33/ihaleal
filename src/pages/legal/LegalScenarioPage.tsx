...
export default function LegalScenarioPage() {
  ...
  const toneCls = {
    emerald: "border-[var(--cizgi)] bg-[var(--zemin-yumusak)] text-[var(--metin-ikincil)]",
    amber: "border-[var(--cizgi)] bg-[var(--zemin-yumusak)] text-[var(--metin-ikincil)]",
    rose: "border-[var(--cizgi)] bg-[var(--zemin-yumusak)] text-[var(--metin-ikincil)]",
  }[tone];
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          ...
          <h1 className="text-3xl md:text-4xl font-normal flex items-center gap-3">
            <BookOpen className="h-7 w-7 text-[var(--metin-ikincil)]" />
        ...
        <section className="rounded-[20px] border border-[var(--cizgi)] bg-slate-900/40 p-5">
          ...
          <p className="text-sm text-slate-300 mb-3 leading-relaxed">
            Gayrimenkul işlemlerinde karşılaşılan <strong className="text-[var(--metin-ikincil)]">10 tipik hukuki/vergisel
          ...
          <div className="grid sm:grid-cols-3 gap-3 text-xs">
            <div className="rounded-[10px] border border-[var(--cizgi)] bg-slate-900/30 p-3">
              <p className="font-normal text-[var(--metin-ikincil)] mb-1">👤 Bireysel</p>
              <p className="text-slate-300">Aile içi devir, miras paylaşımı, boşanma → riskleri görmek.</p>
            </div>
            <div className="rounded-[10px] border border-[var(--cizgi)] bg-slate-900/30 p-3">
              <p className="font-normal text-[var(--metin-ikincil)] mb-1">🏗️ Müteahhit</p>
              <p className="text-slate-300">Kat karşılığı + ipotekli mülk + şirket üzerinden işlem.</p>
            </div>
            <div className="rounded-[10px] border border-[var(--cizgi)] bg-slate-900/30 p-3">
              <p className="font-normal text-[var(--metin-ikincil)] mb-1">🏢 Yatırımcı</p>
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-base font-normal text-white flex items-center gap-2">
              <Layers className="h-4 w-4 text-[var(--metin-ikincil)]" />
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full ps-9 pe-3 py-1.5 rounded-[3px] border border-slate-700 bg-slate-900/60 text-xs placeholder-slate-500 focus:outline-none focus:border-[var(--cizgi)]"
            ...
            {filtered.map((s) => {
              ...
              return (
                <button
                  ...
                  className={`text-start rounded-[20px] border p-4 transition-colors ${isActive ? "border-[var(--cizgi)] bg-[var(--zemin-yumusak)] ring-1 ring-[var(--cizgi)]" : "border-slate-700 bg-slate-900/40 hover:bg-slate-900/60"}`}
                  aria-pressed={isActive}
                >
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <h3 className="text-sm font-normal text-white">{s.title}</h3>
                    {isActive && <CheckCircle2 className="h-4 w-4 text-[var(--metin-ikincil)] flex-shrink-0" />}
                  </div>
          ...
          <section className="space-y-4">
                  ...
                  <tbody className="text-slate-300">
                    {analysis.vergiler.map((v, i) => (
                      <tr key={i} className="border-b border-slate-800/50 last:border-0">
                        ...
                        <td className="py-2 px-2 text-[var(--metin-ikincil)]">{v.oran}</td>
                        <td className="py-2 px-2 text-slate-400">{v.yukumlu}</td>
                        <td className="py-2 ps-2 text-[var(--metin-ikincil)] text-[10px]">{v.mevzuat}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {analysis.sakliPayEtkisi ? (
              <div className="rounded-[20px] border border-[var(--cizgi)] bg-[var(--zemin-yumusak)] p-4">
                <p className="text-xs font-normal text-[var(--metin-ikincil)] mb-1">📋 Saklı Pay Etkisi</p>
            ...
            <div className="rounded-[20px] border border-[var(--cizgi)] bg-slate-900/40 p-5">
              <h3 className="text-base font-normal text-white mb-3 flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-[var(--metin-ikincil)]" />
              ...
              <ol className="space-y-3">
                {analysis.guvenliYol.map((g) => (
                  <li key={g.step} className="flex items-start gap-3">
                    <span className="flex-shrink-0 w-7 h-7 rounded-full bg-[var(--zemin-yumusak)] text-[var(--metin-ikincil)] text-sm font-normal flex items-center justify-center">
                      {g.step}
                    </span>
                    <div className="flex-1 min-w-0">
                      ...
                      {g.sure ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-[var(--zemin-yumusak)] border border-[var(--cizgi)] px-2 py-0.5 text-[10px] text-[var(--metin-ikincil)] mt-1">
                          ⏱️ {g.sure}
                        </span>
                      ) : null}
                    </div>
              ...
              <ul className="space-y-2">
                {analysis.aiYorum.map((y, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-slate-300">
                    <ChevronRight className="rtl:rotate-180 h-3.5 w-3.5 text-[var(--metin-ikincil)] flex-shrink-0 mt-0.5" />
                    <span>{y}</span>
                  </li>
                ))}
              </ul>
              ...
              <div className="rounded-[20px] border border-[var(--cizgi)] bg-slate-900/40 p-5">
                ...
                <ul className="space-y-2">
                  {analysis.ictihat.map((i, idx) => (
                    <li key={idx} className="rounded-[10px] border border-[var(--cizgi)] bg-slate-900/30 p-3 text-xs">
                      <p className="font-mono text-[var(--metin-ikincil)]">{i.daire} {i.karar}</p>
        ...
        <div className="rounded-[20px] border border-slate-700 bg-slate-900/40 p-4 flex items-start gap-3">
          ...
          <div className="text-sm">
            ...
            <p className="text-xs text-slate-400">
              ...
              {" "}<button type="button" onClick={() => navigate("/iletisim")} className="text-[var(--metin-ikincil)] underline">iletişim</button>'e
              veya <a href="mailto:hukuk@ihaleal.com" className="text-[var(--metin-ikincil)] underline">hukuk@ihaleal.com</a>'a yazabilirsiniz.
            </p>
          </div>
        </div>
      </div>

...
function LiveTicker() {
  ...
  return (
    <motion.div
      className="mb-8 overflow-hidden rounded-[20px] border border-white/10 bg-white/[0.03] backdrop-blur-md"
        ...
        <motion.div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
          {MODULES.map((m, i) => (
            <motion.div
              ...
              className={`group flex flex-col rounded-[20px] border p-6 backdrop-blur-xl ${m.accent}`}
            >
              <motion.div className="flex items-start justify-between gap-4">
                <motion.div className="flex h-12 w-12 items-center justify-center rounded-[20px] border border-white/15 bg-black/30">
                  <m.icon className="w-6 h-6 text-white/90" />
                </motion.div>
                <span className="text-[10px] font-normal uppercase tracking-wider text-slate-400 border border-white/10 rounded-full px-2 py-0.5">{m.badge}</span>
              </motion.div>
              <h2 className="text-xl font-normal mt-4">{m.title}</h2>

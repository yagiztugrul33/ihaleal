import { useNavigate } from "react-router-dom";
import {
  ArrowLeft, Crown, Check, X, CheckCircle2, AlertTriangle, RefreshCw, ChevronUp, ChevronDown,
  Calendar, CreditCard, Receipt, ScrollText, Shield, ArrowUpRight,
} from "lucide-react";
import { useMembershipTier } from "@/hooks/useMembershipTier";
import { PRICING_TIERS, type TierId } from "@/lib/pricingTiers";
import { Button } from "@/components/ui/button";

export default function MyMembershipPage() {
  const navigate = useNavigate();
  const { tierId, tier, isPremium, setLocalTier } = useMembershipTier();

  const tierOrder: TierId[] = ["free", "emlak_baslangic", "emlak_pro", "kurumsal"];
  const currentIdx = tierOrder.indexOf(tierId);
  const higherTiers = PRICING_TIERS.filter((t) => tierOrder.indexOf(t.id) > currentIdx);
  const lowerTiers = PRICING_TIERS.filter((t) => tierOrder.indexOf(t.id) < currentIdx);

  return (
    <main className="min-h-screen pt-24 pb-16 px-4 text-white">
      <div className="max-w-4xl mx-auto space-y-6">
        <Button
          variant="ghost"
          size="sm"
          onClick={() => navigate(-1)}
          className="text-slate-500 hover:text-white gap-2"
        >
          <ArrowLeft className="rtl:rotate-180 w-4 h-4" /> Geri
        </Button>

        <div>
          <h1 className="text-3xl font-normal flex items-center gap-2">
            <Crown className="h-7 w-7 text-[var(--metin-ikincil)]" />
            Üyeliğim
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            Mevcut paketin, yükseltme seçenekleri, fatura geçmişi ve hesap yönetimi.
          </p>
        </div>

        {/* Mevcut paket kartı */}
        <div className="rounded-[20px] border p-6" style={isPremium ? { borderColor: "var(--durum-basari)", backgroundColor: "var(--durum-basari-zemin)" } : { borderColor: "var(--cizgi)", backgroundColor: "var(--zemin-yumusak)" }}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
            <div>
              <p className="text-xs text-slate-500 mb-1">Mevcut paketim</p>
              <h2 className="text-2xl font-normal text-white flex items-center gap-2">
                {isPremium ? <Crown className="h-6 w-6" style={{ color: "var(--durum-basari)" }} /> : null}
                {tier.name}
              </h2>
              <p className="text-sm text-slate-400 mt-1">{tier.tagline}</p>
            </div>
            <div className="text-end">
              {tier.customPricing ? (
                <p className="text-3xl font-normal text-white">Özel</p>
              ) : tier.monthlyTry === 0 ? (
                <p className="text-3xl font-normal text-white">Ücretsiz</p>
              ) : (
                <>
                  <p className="text-3xl font-normal text-white">₺{tier.monthlyTry.toLocaleString("tr-TR")}</p>
                  <p className="text-xs text-slate-400">/ay</p>
                </>
              )}
            </div>
          </div>

          {/* Mini özet — ilan limit, ekip, popüler özellikler */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-slate-700">
            <div>
              <p className="text-[10px] text-slate-500 uppercase">İlan limiti</p>
              <p className="text-base font-normal text-white">
                {tier.listingLimit === "unlimited" ? "∞" : `${tier.listingLimit}/yıl`}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase">Ekip üye</p>
              <p className="text-base font-normal text-white">
                {tier.teamSeats === "unlimited" ? "∞" : tier.teamSeats}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase">Borsa Tam</p>
              <p className="text-base font-normal text-white">
                {tier.features.find((f) => /Borsa terminel TAM/i.test(f.label))?.status === "included" ? "✓" : "—"}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase">Gerçek Kapanış</p>
              <p className="text-base font-normal text-white">
                {tier.features.find((f) => /Gerçek kapanış/i.test(f.label))?.status === "included" ? "✓" : "—"}
              </p>
            </div>
          </div>

          {!isPremium && (
            <div className="mt-5 rounded-[10px] border border-[var(--cizgi)] bg-[var(--zemin-yumusak)] p-3 flex items-center gap-3">
              <Crown className="h-5 w-5 text-[var(--metin-ikincil)] flex-shrink-0" />
              <p className="text-xs text-white/90 flex-1">
                Premium paketler ile borsa terminel + sınırsız rapor + gerçek kapanış verisi açılır.
              </p>
              <Button size="sm" onClick={() => navigate("/fiyatlandirma")}>
                Yükselt
              </Button>
            </div>
          )}
        </div>

        {/* Yükselt / Düşür seçenekleri */}
        {(higherTiers.length > 0 || lowerTiers.length > 0) && (
          <div className="rounded-[20px] border border-slate-700 bg-slate-900/40 p-5">
            <h3 className="text-base font-normal text-white mb-3 flex items-center gap-2">
              <RefreshCw className="h-4 w-4 text-[var(--metin-ikincil)]" />
              Paket Değiştir
            </h3>

            {higherTiers.length > 0 && (
              <div className="mb-4">
                <p className="text-xs font-normal mb-2 flex items-center gap-1.5" style={{ color: "var(--metrik-yesil)" }}>
                  <ChevronUp className="h-3.5 w-3.5" /> Yükselt
                </p>
                <div className="grid sm:grid-cols-2 gap-2">
                  {higherTiers.map((t) => (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => navigate(`/odeme/baslat?paket=${t.id}&periyot=monthly`)}
                      className="text-start rounded-[10px] border border-slate-700 bg-slate-900/60 hover:bg-slate-800/60 p-3 transition-colors"
                    >
                      <p className="text-sm font-normal text-white">{t.name}</p>
                      <p className="text-xs text-slate-400 line-clamp-1">{t.tagline}</p>
                      <p className="text-xs mt-1" style={{ color: "var(--metrik-yesil)" }}>
                        {t.customPricing ? "Özel" : `₺${t.monthlyTry.toLocaleString("tr-TR")}/ay`}
                      </p>
                    </button>
                  ))}
                </div>
              </div>
            )}

            {lowerTiers.length > 0 && (
              <div>
                <p className="text-xs text-slate-400 font-normal mb-2 flex items-center gap-1.5">
                  <ChevronDown className="h-3.5 w-3.5" /> Düşür
                </p>
                <div className="grid sm:grid-cols-2 gap-2">
                  {lowerTiers.map((t) => (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => navigate(`/odeme/baslat?paket=${t.id}&periyot=monthly`)}
                      className="text-start rounded-[10px] border border-slate-700 bg-slate-900/60 hover:bg-slate-800/60 p-3 transition-colors"
                    >
                      <p className="text-sm font-normal text-white">{t.name}</p>
                      <p className="text-xs text-slate-400 line-clamp-1">{t.tagline}</p>
                      <p className="text-xs text-slate-300 mt-1">
                        {t.monthlyTry === 0 ? "Ücretsiz" : `₺${t.monthlyTry.toLocaleString("tr-TR")}/ay`}
                      </p>
                    </button>
                  ))}
                </div>
              </div>
            )}
          </div>
        )}

        {/* Fatura geçmişi (UI iskelet) */}
        <div className="rounded-[20px] border border-slate-700 bg-slate-900/40 p-5">
          <h3 className="text-base font-normal text-white mb-3 flex items-center gap-2">
            <Receipt className="h-4 w-4 text-[var(--metin-ikincil)]" />
            Fatura Geçmişi
          </h3>
          {isPremium ? (
            <ul className="divide-y divide-slate-800 text-sm">
              <li className="flex items-center justify-between py-3">
                <div>
                  <p className="text-slate-300">Mayıs 2026 — {tier.name}</p>
                  <p className="text-xs text-slate-500 flex items-center gap-1">
                    <Calendar className="h-3 w-3" /> 01.05.2026 · Kart **** 4242
                  </p>
                </div>
                <div className="text-end">
                  <p className="font-normal" style={{ color: "var(--durum-basari)" }}>₺{tier.monthlyTry.toLocaleString("tr-TR")}</p>
                  <p className="text-xs text-slate-500">Ödendi</p>
                </div>
              </li>
              <li className="flex items-center justify-between py-3">
                <div>
                  <p className="text-slate-300">Nisan 2026 — {tier.name}</p>
                  <p className="text-xs text-slate-500 flex items-center gap-1">
                    <Calendar className="h-3 w-3" /> 01.04.2026 · Kart **** 4242
                  </p>
                </div>
                <div className="text-end">
                  <p className="font-normal" style={{ color: "var(--durum-basari)" }}>₺{tier.monthlyTry.toLocaleString("tr-TR")}</p>
                  <p className="text-xs text-slate-500">Ödendi</p>
                </div>
              </li>
            </ul>
          ) : (
            <p className="empty-state text-sm">
              <Receipt className="mx-auto mb-2 h-8 w-8 opacity-50" />
              Henüz faturan yok. Premium pakete geçince burada görünür.
            </p>
          )}
        </div>

        {/* Ödeme yöntemi */}
        <div className="rounded-[20px] border border-slate-700 bg-slate-900/40 p-5">
          <h3 className="text-base font-normal text-white mb-3 flex items-center gap-2">
            <CreditCard className="h-4 w-4 text-[var(--metin-ikincil)]" />
            Ödeme Yöntemi
          </h3>
          {isPremium ? (
            <div className="flex items-center justify-between rounded-[10px] bg-slate-900/60 p-3 border border-slate-700">
              <div className="flex items-center gap-3">
                <div className="rounded-[3px] bg-[var(--zemin)] px-2 py-1 text-[var(--metin-ikincil)] text-xs font-normal">VISA</div>
                <div>
                  <p className="text-sm text-slate-200">**** **** **** 4242</p>
                  <p className="text-[10px] text-slate-500">12 / 28</p>
                </div>
              </div>
              <button type="button" className="text-xs text-[var(--metin-ikincil)] hover:underline">
                Değiştir
              </button>
            </div>
          ) : (
            <p className="text-sm text-slate-400">Henüz kayıtlı kart yok.</p>
          )}
        </div>

        {/* Tier dev/demo toggler (sadece UI test) */}
        <details className="rounded-[20px] border border-[var(--cizgi)] bg-[var(--zemin-yumusak)] p-3">
          <summary className="cursor-pointer text-xs text-[var(--metin-ikincil)] font-normal flex items-center gap-2">
            <AlertTriangle className="h-3.5 w-3.5" />
            Geliştirici / Demo — Tier Değiştir (UI test amaçlı)
          </summary>
          <div className="mt-3 flex flex-wrap gap-2">
            {PRICING_TIERS.map((t) => (
              <button
                key={t.id}
                type="button"
                onClick={() => setLocalTier(t.id)}
                className={`text-xs rounded-[3px] px-3 py-1.5 border ${tierId === t.id ? "bg-[var(--metin)] text-[var(--zemin)] border-[var(--metin)]" : "border-slate-700 text-slate-300 hover:bg-slate-800"}`}
              >
                {t.name}
              </button>
            ))}
          </div>
          <p className="text-[10px] text-slate-500 mt-2">
            Bu seçim sadece tarayıcında saklanır (localStorage). Gerçek tier Supabase'den okunur.
          </p>
        </details>

        {/* İptal + Yasal */}
        <div className="rounded-[20px] border border-slate-700 bg-slate-900/40 p-5">
          <h3 className="text-base font-normal text-white mb-3 flex items-center gap-2">
            <Shield className="h-4 w-4 text-[var(--metin-ikincil)]" />
            Hesap Yönetimi
          </h3>
          <ul className="space-y-2 text-sm">
            <li className="flex items-center justify-between">
              <span className="text-slate-300">Aboneliği iptal et</span>
              <button
                type="button"
                onClick={() => navigate("/abone/iptal")}
                className="text-xs hover:underline"
                style={{ color: "var(--durum-hata)" }}
              >
                İptal akışı →
              </button>
            </li>
            <li className="flex items-center justify-between">
              <span className="text-slate-300">KVKK + Aydınlatma metni</span>
              <button
                type="button"
                onClick={() => navigate("/kvkk")}
                className="text-xs text-[var(--metin-ikincil)] hover:underline"
              >
                Görüntüle →
              </button>
            </li>
            <li className="flex items-center justify-between">
              <span className="text-slate-300">Fiyatlandırma + tüm paketler</span>
              <button
                type="button"
                onClick={() => navigate("/fiyatlandirma")}
                className="text-xs text-[var(--metin-ikincil)] hover:underline flex items-center gap-1"
              >
                Görüntüle <ArrowUpRight className="h-3 w-3" />
              </button>
            </li>
          </ul>
        </div>

        <p className="text-[10px] text-slate-500 text-center pt-2">
          Tüm üyelik işlemleri 6098 BK + 6502 TKHK + 6698 KVKK uyumludur. Sorular: <a href="mailto:destek@ihaleal.com" className="underline">destek@ihaleal.com</a>
        </p>
      </div>
    </main>
  );
}

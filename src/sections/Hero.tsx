    ...
    <section id="hero" className="relative flex min-h-screen items-center justify-center overflow-hidden pt-20">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-24 xl:py-28">
        <div className="grid items-start gap-10 lg:grid-cols-2 lg:gap-16 xl:gap-20">
          {/* lg: solda istatistik ızgarası; mobilde altta (order-2) */}
          <div className="order-2 grid w-full max-w-md grid-cols-2 justify-self-stretch gap-2.5 sm:gap-3 lg:order-1 lg:max-w-none lg:justify-self-start xl:max-w-lg animate-fade-in-up animate-delay-500">
            {stats.map((s, i) => {
              const Icon = s.icon;
              return (
                <div
                  key={i}
                  className="group relative flex min-h-[104px] flex-col overflow-hidden rounded-xl border border-sky-400/15 bg-white/[0.04] p-3 backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:border-sky-400/35 hover:shadow-[0_12px_32px_-12px_rgba(14,165,233,0.18)] motion-reduce:hover:translate-y-0 sm:min-h-[112px] sm:p-3.5"
                >
                  <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100 motion-reduce:opacity-0 bg-gradient-to-br from-sky-400/[0.06] to-transparent" />
                  <div className="pointer-events-none absolute right-0 top-0 h-16 w-16 rounded-full bg-sky-400/[0.08] blur-xl opacity-60 transition-opacity duration-300 group-hover:opacity-90 sm:h-20 sm:w-20" />
                  <div className="relative mb-2 flex items-start justify-between">
                    <div className="rounded-lg bg-black/30 p-1.5 text-sky-200/90 ring-1 ring-sky-400/20 transition-transform duration-200 group-hover:scale-105 motion-reduce:group-hover:scale-100">
                      <Icon className="h-4 w-4" aria-hidden />
                    </div>
                  </div>
                  <div className="relative mt-auto flex min-h-[2rem] items-end sm:min-h-[2.25rem]">
                    <span className="text-lg font-bold tabular-nums tracking-tight text-zinc-50 drop-shadow-sm sm:text-xl">
                      {s.value}
                    </span>
                  </div>
                  <div className="relative mt-1 text-[10px] font-medium leading-snug text-sky-100/75 sm:text-[11px]">
                    {s.label}
                  </div>
                </div>
              );
          ...
          {/* lg: sağda metin kolonu; mobilde üstte (order-1) */}
          <div className="order-1 flex w-full max-w-xl flex-col items-start text-left sm:max-w-2xl lg:order-2 lg:max-w-xl lg:border-l lg:border-sky-400/15 lg:pl-12 xl:pl-14">
            <h1 className="animate-fade-in-up animate-delay-200 mb-5 w-full max-w-xl text-4xl font-bold leading-[1.08] tracking-tight text-zinc-50 sm:text-5xl lg:text-6xl xl:text-[3.5rem] xl:leading-[1.06]">
              <span className="block text-zinc-50">Gayrimenkul</span>
              <span className="mt-2 block font-semibold tracking-tight text-sky-400">
                Canlı İhale Vitrini
              </span>
            </h1>
            <p className="animate-fade-in-up animate-delay-300 mb-6 w-full max-w-xl text-pretty text-base font-normal leading-relaxed text-zinc-100/90 sm:text-lg">
              Şeffaf, hızlı ve güvenli ihale deneyimi. <strong className="font-semibold text-sky-300">İhaleal Endeksi</strong> ile teklif yoğunluğu, bölge bandı ve yapay zeka sinyallerini bir arada görün; gerçekçi karar için yatırım skoru ve senaryolar tek ekranda (demo veri).
            </p>
            <div
              className="animate-fade-in-up animate-delay-300 mb-7 flex w-full max-w-xl items-center gap-2 border-t border-sky-400/20 pt-5"
              role="note"
            >
            ...
            <div className={cn(HERO_ACTION_ROW, "mt-2.5 animate-fade-in-up animate-delay-400")}>
              <Button
                ...
                className={heroActionShell(
                  "border border-sky-400/25 bg-white/[0.04] text-zinc-100 shadow-sm hover:border-sky-400/45 hover:bg-sky-500/[0.06]"
                )}
              >
            ...
            <p className="mt-4 w-full max-w-xl rounded-lg border border-sky-400/15 bg-sky-500/[0.04] px-3 py-2.5 text-left text-[11px] font-medium leading-relaxed text-sky-100/85 shadow-inner shadow-black/20 sm:text-xs">
      ...
      <div className="absolute bottom-8 left-1/2 flex -translate-x-1/2 flex-col items-center gap-2 animate-fade-in animate-delay-700">
        <span className="text-[10px] font-semibold uppercase tracking-[0.35em] text-sky-200/70">Kaydır</span>
        <div className="flex h-8 w-5 items-start justify-center rounded-full border border-sky-400/35 bg-sky-500/[0.06] p-1 ring-1 ring-sky-400/15 backdrop-blur-sm">

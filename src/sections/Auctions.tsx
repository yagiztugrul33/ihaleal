            ...
            <Card key={auction.id} className={`property-card group flex h-full flex-col overflow-hidden card-luxury !p-0 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: `${idx * 100}ms` }}>
              ...
              <CardContent className="flex flex-1 flex-col p-5">
                {(() => {
                  ...
                  return (
                    <div className="mb-2 flex items-center justify-between text-xs">
                      ...
                      <span className={`font-semibold ${changeTone}`}>
                        {changePct >= 0 ? "+" : ""}
                        %{changePct.toFixed(2)}
                      </span>
                    </div>
                ...
                <div className="mt-2 flex flex-wrap gap-1.5 text-xs">
                  <span className="rounded-md bg-slate-800/60 px-2 py-1 text-slate-200">{auction.propertyDetails.grossSqm} m²</span>
                  <span className="rounded-md bg-slate-800/60 px-2 py-1 text-slate-200">{auction.propertyDetails.roomCount}</span>
                  <span className="rounded-md bg-slate-800/60 px-2 py-1 text-slate-200">{auction.city}</span>
                </div>
                      ...
                      {isHome ? (
                        <>
                          <button
                            ...
                            className="btn-ghost text-xs h-8 px-3 inline-flex items-center"
                          >
                            İzle
                          </button>
                        ...
                        <>
                          <Button size="sm" variant="outline" onClick={(e) => handleFavoriteClick(e, auction.id)} className="border-white/10 text-slate-300 hover:text-white hover:bg-white/5 text-xs h-8 px-3">İzle</Button>

...
const STAT_ICONS = ["\u{1F4CA}", "\u2705", "\u{1F465}", "\u2B50"] as const;
const TRUST_ICONS = ["\u{1F6E1}\uFE0F", "\u{1F465}", "\u2B50", "\u{1F3A7}"] as const;
const STEP_ICONS = ["\u{1F50D}", "\u2705", "\u{1F4B0}", "\u{1F3C6}"] as const;
const STEP_NOS = ["01", "02", "03", "04"] as const;
const AUCTION_TONES = ["villa", "office", "apartment", "complex"] as const;
const CATEGORY_META = [
  { icon: HomeIcon, href: "/ilanlar?tip=villa" },
  { icon: Building2, href: "/ilanlar?tip=daire" },
  { icon: Landmark, href: "/ilanlar?tip=ofis" },
  { icon: MapPin, href: "/ilanlar?tip=arsa" },
  { icon: Warehouse, href: "/ilanlar?tip=depo" },
  { icon: Building2, href: "/ilanlar?tip=komple" },
] as const;

const TESTIMONIAL_AVATARS = [
  "https://ui-avatars.com/api/?name=Ayse+Korkmaz&background=3b82f6&color=fff",
  "https://ui-avatars.com/api/?name=Mehmet+Yildiz&background=0ea5e9&color=fff",
  "https://ui-avatars.com/api/?name=Zeynep+Arslan&background=6366f1&color=fff",
] as const;
...
export default function PremiumCinematicHome() {
  ...
  const auctions = useMemo(
    () =>
      home.auctions.items.map((item, i) => ({
        title: item.title,
  ...
  const categories = useMemo(
    () =>
      home.categories.items.map((item, i) => ({
        name: item.name,
        count: item.count,
        icon: CATEGORY_META[i]?.icon ?? Building2,
        href: CATEGORY_META[i]?.href ?? "/ilanlar",
      })),
  ...
  const investorItems = useMemo(
    () =>
      home.investor.items.map((item, i) => ({
        ...item,
        icon: ["\u{1F6E1}\uFE0F", "\u{1F916}", "\u{1F4C4}", "\u{1F30D}"][i] ?? "\u{1F6E1}\uFE0F",
      })),
    [home.investor.items],
  );
            ...
            <div className="premium-auction-grid">
              {auctions.map((auction) => (
                <article key={auction.title} className="premium-auction-card">
                  ...
                  >
                    <span>â— {home.auctions.live}</span>
                  </div>
                  <div className="premium-auction-card__body">
                    <h3>{auction.title}</h3>
                    <p>ğŸ“ {auction.location}</p>
                    <div className="premium-auction-card__bid">
                      <div>
                        <small>{home.auctions.currentBid}</small>
        ...
        <div className="premium-testimonials__grid">
          {testimonials.map((item) => (
            <article key={item.name} className="premium-testimonial-card">
              <img src={item.avatar} alt="" className="premium-testimonial-card__avatar" />
              <div
                className="premium-testimonial-card__stars"

        ...
        <div className="grid lg:grid-cols-5 gap-10 lg:gap-12">
          <div className="lg:col-span-2 space-y-6">
            {[
              ...
              <div key={idx} className={`flex items-start gap-4 p-4 rounded-xl bg-slate-900/40 border border-white/5 hover:border-white/10 transition-all duration-500 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-6"}`} style={{ transitionDelay: `${idx * 100}ms` }}>
          ...
          <form onSubmit={handleSubmit} className={`lg:col-span-3 p-6 lg:p-8 rounded-2xl bg-slate-900/40 border border-white/5 space-y-5 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: "200ms" }}>
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              <div><label className="text-sm text-slate-400 mb-1.5 block">Ad Soyad</label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={`bg-slate-950 border ${errors.name ? "border-red-500/50" : "border-white/10"} text-white focus:ring-blue-500/50`} placeholder="Ali Veli" />{errors.name && <p className="text-xs text-red-400 mt-1">{errors.name}</p>}</div>
              <div><label className="text-sm text-slate-400 mb-1.5 block">E-posta</label><Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className={`bg-slate-950 border ${errors.email ? "border-red-500/50" : "border-white/10"} text-white focus:ring-blue-500/50`} placeholder="ali@ornek.com" />{errors.email && <p className="text-xs text-red-400 mt-1">{errors.email}</p>}</div>
            </div>
            <div><label className="text-sm text-slate-400 mb-1.5 block">Telefon</label><Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className={`bg-slate-950 border ${errors.phone ? "border-red-500/50" : "border-white/10"} text-white focus:ring-blue-500/50`} placeholder="+90 555 123 45 67" />{errors.phone && <p className="text-xs text-red-400 mt-1">{errors.phone}</p>}</div>
            <div><label className="text-sm text-slate-400 mb-1.5 block">Mesajınız</label><Textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className={`bg-slate-950 border ${errors.message ? "border-red-500/50" : "border-white/10"} text-white focus:ring-blue-500/50 min-h-[120px]`} placeholder="Mesajınızı buraya yazın..." />{errors.message && <p className="text-xs text-red-400 mt-1">{errors.message}</p>}</div>
